...
<head><title><?php
    echo !isset($_GET['folder']) ? "index" : 
        $_GET['folder'];
?></title>
<style type="text/css">
html body{background:black;color:white;margin:0px!important;padding:0px!important;font-size:0;<?php if(isset($_GET['selected'])) { echo "overflow:hidden;"; } ?>}
a{color:#CCCCCC;} a:hover{color:green;}
#content{font-size:20pt;}
hr{margin:5px;padding:0px;}
h2{margin:0px!important;}
#media {margin-right:40px;}
.navitem {width:19%;border:1px solid white;margin:0px;padding:0px;height:40px;
text-align:center;vertical-align:middle;display:inline-block;font-size:20pt;}
<?php if(isset($_GET['selected'])) { echo
"#nav{font-size:0;padding:0px;position:fixed;
left:calc(100% - 40px);width:100vh;transform-origin:left top;background:black;
opacity:0.2;transform:rotate(90deg) translateY(-100%);height:40px;margin:0px;}
#nav:hover{opacity:1.0;}" ; } ?>
</style>
<script type="text/javascript">
window.onload = window.onresize = function() { resize(); }
window.onkeydown = function(e) { keypresses(); }
function keypresses()
{
    if(shiftPressed()) { return; }
    var key = window.event.keyCode; var tgt = "";
    switch(key)
    {
        case 37:  // left arrow; previous pic
            tgt = "prev"; break;
        case 38:  // up arrow; folder contents
            tgt = "index"; break;
        case 39:  // right arrow; next pic
            tgt = "next"; break;
        ...
        case 109:  // "num -" video vol down
            tgt = "voldn"; break;
        case 219: // "[" character; first pic in list
            tgt = "firstpic"; break;
        case 221: // "]" character; last pic in list
            tgt = "lastpic"; break;
        case 191: // "/" (slash) character; random pic from list
            randomlink(); return;
        case 220: // "\" (backslash) character; return link menu
    ...
    else
    {
        ...
        if(tgt == "volup")
        {
            if(md.muted == true) { md.muted = false; return; }
            if(md.volume < 1.0) { md.volume = md.volume + 0.1; }
        }
        if(tgt == "voldn")
        {
            if(md.volume > 0.0)
...
function randomlink()
{
    ...
    if(elems.length == 0)
    {
        ...
        if(elems.length == 0) { alert("nothing to show"); return; }
    }
...
function resize()
{
    if(document.getElementById("media") == null) { return; }
...
<body style="padding:0px 10px;" align="center"><div id="nav"><div class="navitem"><a href="http://<?php echo $_SERVER['HTTP_HOST']; ?>">home</a></div><?php if(!isset($_GET['folder'])) { echo "<hr />";  } ?>
<?php
    ...
    if(!isset($curfolder))
    {
        ...
        closedir($dh);
        ?>    <h2 align="center" id="content"><a href="javascript:void(0);" onclick="randomlink();">???</a> | <?php
        $inactive = false;
        for($folderit = 0; $folderit < sizeof($folderlist); $folderit++)
        { ?> 
                <a class="folderlink" href="./?folder=<?php echo $folderlist[$folderit];
                ?>"><?php $folderlist[$folderit]; ?></a>
                <?php if($folderit + 1 < sizeof($folderlist))
                { echo " | "; } 
                ?>
        <?php }  ?></h2><?php
    }
    else
    {
        ...
        ?>
            <div class="navitem"><a href="./">list</a></div><?php if(!isset($_GET['selected'])) { echo "<hr />";  } ?>
        <?php
        if(!isset($_GET['selected']) || !in_array($_GET['selected'], $filelist))
        {
            $videos = isset($filelist2) ? sizeof($filelist2) : 0;
            echo '<div id="content"><a href="javascript:void(0);" onclick="randomlink();">???</a> | ';
            for($it = 0; $it < sizeof($filelist) - $videos; $it++)
            {?>
                <a class="piclink" href="./?folder=<?php echo $curfolder;
                ?>&amp;selected=<?php echo $filelist[$it]; ?>"<?php if($it == 0)
            ...
            if(isset($filelist2) && sizeof($filelist2) > 0)
            {
                ...
                {?>
                    <a class="piclink" href="./?folder=<?php echo $curfolder;
                    ?>&amp;selected=<?php echo $filelist2[$it]; ?>"<?php if($it == 0)
                    { echo ' id="firstpic"'; } else if($it == sizeof($filelist2) - 1)
                    { echo ' id="lastpic"'; } ?>><?php echo ($it + 1); ?></a>
                <?php 
        ...
        else
        {
            ...
            ?><div class="navitem"><a<?php
            if($key > 0)
            {
                echo ' href="./?folder=' . $curfolder . '&amp;selected=' . 
                $filelist[$key - 1] . '"'; 
            } ?> id="prev">prev</a></div><div class="navitem"><a href="./?folder=<?php 
            echo $curfolder; ?>" id="index">index</a></div><div class="navitem"><a<?php if($key < sizeof($filelist) - 1)
            {
                echo ' href="./?folder=' . $curfolder . '&amp;selected=' . $filelist[$key + 1] . '"';
            } ?> id="next">next</a></div></div>
            
            <?php $vidext = strtolower(substr($current, strlen($current)-4));
            
            if($vidext != ".mp4" && $vidext != "webm" && $vidext != ".avi"
                && $vidext != ".mov"  && $vidext != ".wmv" && $vidext != ".flv" ) { ?>
                <img id="media" name="media" src="<?php echo $curfolder . "/" . $_GET['selected']; ?>" />
            <?php } else {
                switch($vidext)
                { 
                    case ".mp4":
            ...
            ?>
                <video id="media" width="800" height="600" controls loop autoplay>
                    <source src="<?php echo $curfolder . "/" . $current; ?>"
                    type="video/<?php echo $vidext == "webm" ? $vidext : substr($vidext, 1) ?>" >
                </video>
            <?php 
                break;
                default: 
                    die("error loading video");
                }
            }
        }
    }
